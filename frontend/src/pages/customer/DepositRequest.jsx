...
const DepositRequest = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedAccountId) {
      showToast('Please select an account for deposit.', 'error');
      return;
    }
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs text-[var(--finova-text-secondary)]">
          ...
          <span>/</span>
          <span className="font-semibold text-[var(--finova-text-heading)]">Request Deposit</span>
        </div>
        ...
        <Card className="overflow-hidden">
          <CardHeader className="bg-[var(--finova-bg-secondary)] border-b border-[var(--finova-border)]">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-[var(--finova-mint)] text-[var(--finova-primary)] flex items-center justify-center font-bold shadow-xs">
                <ArrowDownLeft className="h-5 w-5" />
              </div>
              <div>
                <CardTitle className="text-xl">Request a Deposit</CardTitle>
                <CardDescription>
                  Submit a deposit request. A Finova administrator will review and process it.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="p-6 md:p-8 space-y-6">
            ...
            <div className="rounded-xl p-4 bg-[var(--finova-info-bg)] border border-[var(--finova-info)]/20 flex items-start gap-3">
              ...
              <div className="text-xs text-[var(--finova-text-secondary)] space-y-1">
                <p className="font-bold text-[var(--finova-text-heading)]">Institutional Verification Policy</p>
                <p>
                  To prevent unauthorized balance modifications, all customer cash and counter deposits require administrative verification before ledger posting. Your account balance will update automatically once verified by an institutional officer.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {accounts.length === 0 ? (
                  <div className="p-4 rounded-xl border border-[var(--finova-warning)]/30 bg-[var(--finova-warning-bg)] text-xs text-[var(--finova-warning)]">
                    No active bank accounts found eligible for deposits.
                  </div>
                ) : (
              ...
              <div>
                ...
                <div className="relative rounded-xl shadow-xs">
                  ...
                  <input
                    ...
                    className="block w-full rounded-xl border border-[var(--finova-border)] bg-[var(--finova-card-bg)] pl-9 pr-4 py-3.5 text-base font-bold text-[var(--finova-text-heading)] placeholder:text-[var(--finova-text-muted)] focus:border-[var(--finova-primary)] focus:ring-1 focus:ring-[var(--finova-primary)] transition-all"
                  />
                </div>
                {amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 && (
                  <p className="mt-2 text-xs font-semibold text-[var(--finova-primary)] flex items-center gap-1.5">
                    <span>Formatted Amount:</span>
                    <span className="font-bold underline">{formatCurrency(parseFloat(amount))}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., Cash counter deposit, branch wire, payroll top-up"
                  maxLength={200}
              ...
              <div className="p-4 rounded-xl bg-[var(--finova-bg-secondary)] border border-[var(--finova-border)] text-xs text-[var(--finova-text-secondary)] flex items-center gap-3">
                ...
                <span>
                  <strong>Important Notice:</strong> Your balance will not change immediately upon submission. An administrator will review and authorize the deposit.
                </span>
              </div>
        ...
        <Card className="overflow-hidden border-2 border-emerald-500/30">
          <div className="bg-gradient-to-r from-emerald-600 to-teal-700 p-8 text-white text-center">
            ...
            <p className="text-sm text-emerald-100 mt-1 max-w-md mx-auto">
              Institutional ticket created. A Finova administrator will review and process your cash deposit.
            </p>
          </div>
          ...
          <CardContent className="p-6 md:p-8 space-y-6">
            <div className="rounded-2xl bg-[var(--finova-bg-secondary)] border border-[var(--finova-border)] p-5 space-y-3">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-[var(--finova-border)]">
                ...
                <span className="text-xs font-bold uppercase px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
            ...
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-700 dark:text-amber-300">
              <strong>Notice:</strong> Your customer balance has <strong>NOT</strong> been modified yet. It will update atomically once the request is approved.
            </div>
